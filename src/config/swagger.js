      ...
      schemas: {
        User: {
          ...
          properties: {
            ...
            is_verified: { type: 'boolean', description: 'User verification status (legacy)' },
            verification_status: { type: 'string', enum: ['pending', 'approved', 'rejected'], description: 'Current verification status' },
            rejection_reason: { type: 'string', nullable: true, description: 'Reason for rejection if status is rejected' },
            verification_submitted_at: { type: 'string', format: 'date-time', nullable: true, description: 'When verification documents were submitted' },
            verification_reviewed_at: { type: 'string', format: 'date-time', nullable: true, description: 'When verification was reviewed by admin' },
        ...
        ServiceProvider: {
          ...
          properties: {
            ...
            provider_isVerified: { type: 'boolean', description: 'Provider verification status (legacy)' },
            verification_status: { type: 'string', enum: ['pending', 'approved', 'rejected'], description: 'Current verification status' },
            rejection_reason: { type: 'string', nullable: true, description: 'Reason for rejection if status is rejected' },
            verification_submitted_at: { type: 'string', format: 'date-time', nullable: true, description: 'When verification documents were submitted' },
            verification_reviewed_at: { type: 'string', format: 'date-time', nullable: true, description: 'When verification was reviewed by admin' },
